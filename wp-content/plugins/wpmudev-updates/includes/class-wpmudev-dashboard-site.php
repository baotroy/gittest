...
class WPMUDEV_Dashboard_Site {
	...
	public function logout( $redirect = true ) {
		...
		if ( $redirect ) {
			...
			$urls = WPMUDEV_Dashboard::$ui->page_urls;
			header( 'X-Redirect-From: SITE logout' );
	...
	public function get_project_infos( $pid, $fetch_full = false ) {
		...
		if ( ! isset( $ProjectInfos[ $pid ] ) ) {
			...
			if ( empty( $remote ) ) {
				error_log(
					sprintf(
						'[WPMUDEV Site Warning] No remote data found for project %s',
						$pid
					)
				);
	...
	public function process_auto_upgrade() {
		...
		foreach ( $scheduled as $pid ) {
			...
			$msg = sprintf(
				'[WPMUDEV Info] Project Auto-Upgrade result for %s: %s',
				$pid,
				($res ? 'Done!' : 'Failed...')
			);
			error_log( $msg, 0 );
	...
	protected function refresh_local_projects( $no_updates_check = false ) {
		...
		$local_projects = $projects = $this->scan_fs_local_projects();
