...
class FeedWordPress {
	...
	public function update ($uri = null, $crash_ts = null) {
		global $wpdb;
		$cf = WP_CONTENT_DIR .'/debug.log';
		if (FeedWordPress::needs_upgrade()) : // Will make duplicate posts if we don't hold off
			return NULL;
		endif;

		// Check running
		# *		Get feedwordpress_running_flag option
		#
		# *		If feedwordpress_running_flag option is not available,
		#		insert it into Options table with value is 1,
		#		and update feedwordpress_last_running value is current timestamp.
		#		And continue to update RSS.
		#
		# *		If feedwordpress_running_flag is 0,
		# 		update feedwordpress_running_flag value to 1 
		#		and update feedwordpress_last_running value is current timestamp. 
		#		And continue to update RSS.
		#
		# *		If feedwordpress_running_flag value is 1,
		# 		check time span between now and feedwordpress_last_running value,
		#		if time span is shorter than feedwordpress_update_window (delay time between updates) value then stop.
		# *		Otherwise time span is longer than feedwordpress_update_window value (wait too long) then continue to update RSS.
		#

		// Generate processing hash
		$processing_hash = uniqid(true, true);
		
		file_put_contents($cf, 'process hash:'."\n".$processing_hash."\n", FILE_APPEND);
		$running_flag = get_option('feedwordpress_running_flag');
		if(!$running_flag){// Not running
			update_option('feedwordpress_running_flag', 1);	
		}
		else{
			$last_running = (int) get_option('feedwordpress_last_running');
			$timeSpan = time() - $last_running;
			$updateWindow = (int) get_option('feedwordpress_update_window', DEFAULT_UPDATE_PERIOD) * 60 /* sec/min */;

			if($timeSpan < $updateWindow){ // Not wait too long yet
				file_put_contents($cf, 'STOP OTHER RUNNING: '.$processing_hash."\n", FILE_APPEND);
				$delta = array('new' => 0, 'update' => 0, 'stored' => 0);
				do_action('feedwordpress_update_complete', $delta);
				return $delta;
			}

			// * Currently feedwordpress_running_flag = 1, so do not need to set it = 1.

			// * Delete all run before
			$wpdb->query("DELETE FROM $wpdb->options WHERE option_name LIKE 'feedwordpress_running_hash%'");
			file_put_contents($cf, 'CONTINUE WAIT TOO LONG: '.$processing_hash."\n", FILE_APPEND);
		}
		
		// Initialize new process
		update_option('feedwordpress_last_running', time());
		update_option('feedwordpress_running_hash_'.$processing_hash, time());
		...
		foreach ($feed_set as $feed_id) :
			...
			if ($pinged_that and $timely) :
				...
				// Check hash again to ensure this process is updating
				// Delay 200 miliseconds
				//usleep(200000);

				# In case multiple requests run at the same time.
				# Get the first request from request list,
				# Process the first request.
				# Ignore other requests

				$first_hash = $wpdb->get_row( "SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'feedwordpress_running_hash%' ORDER BY option_id ASC LIMIT 1" );
								
				file_put_contents($cf, 'running_hash has:'."\n".$processing_hash."\t".$running_hash."\n", FILE_APPEND);

				// Stop processing if its hash is not matched running hash
				$running_hash = 'feedwordpress_running_hash_'.$processing_hash;
				if(strcmp($first_hash->option_name, $running_hash) != 0){ //running request is not first request

					file_put_contents($cf, 'STOP NOT FIRST: '.$processing_hash."\n", FILE_APPEND);

					// Delete self-process hash
					$wpdb->query("DELETE FROM $wpdb->options WHERE option_name = '$running_hash'");
		...
		// delete all running hash
		$wpdb->query("DELETE FROM $wpdb->options WHERE option_name LIKE 'feedwordpress_running_hash%'");

		// Update running flag to 0.
		update_option('feedwordpress_running_flag', 0);	
